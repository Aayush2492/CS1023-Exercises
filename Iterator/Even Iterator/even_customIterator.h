...
public:
    ...
    void push_value(const T &val)
    {
        ...
        else //This will be excecuted only when first element is pushed
        {
            head = p;
        }
        tail = p;
    ...
    {
        if (head->next != nullptr)                          //More than one element
            return *(new customDS_iterator<T>(head->next)); //
